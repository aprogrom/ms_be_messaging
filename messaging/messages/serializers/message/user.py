...
class MessageAnswersDetailSerializer(ModelSerializer):
    answers = MessageListSerializer(many=True)
...
class MessageCreateSerializer(ModelSerializer):
    ...
    def validate_answer(self, value: Message):
        if value.is_delete:
            raise ValidationError('Невозможно ответить на удаленное сообщение')
        return value

    def validate_forwarded(self, value: Message):
        if value.is_delete:
            raise ValidationError('Невозможно цетировать удаленное сообщение')
        return value
